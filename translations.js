const translations = {
    "de-de": {
        ...
        "tech": {
            ...
            "content": "<h3>Architektur der nächsten Generation</h3><p>Wir vereinen modernste Web3-Software mit robuster Hardware, um eine durchgängig sichere, europäische Export-Autobahn aufzubauen. Als SaaS-Provider kümmern wir uns komplett um die Backend-Komplexität, während Sie Compliance-Risiken eliminieren und neue Umsätze generieren.</p><hr class='border-white/10 my-6'><h3>Der Tech-Stack im Detail</h3><h4>1. NXP NTAG 424 DNA (Physischer Anker)</h4><p>QR-Codes sind wertlos gegen professionelle Produktpiraterie. Wir nutzen kryptographisch gesicherte NFC-Chips (AES-128) von NXP. Jedes Produkt generiert beim Auslesen einen einzigartigen, dynamischen Krypto-Code. So schlagen wir die fälschungssichere Brücke zwischen physischer Ware und dem Digitalen Zwilling (DPP).</p><h4>2. Base L2 Blockchain (Sicheres Ledger)</h4><p>Die Identität der Produkte wird auf <strong>Base (Ethereum Layer 2)</strong> verewigt. Dies sorgt für beispiellose Skalierbarkeit bei extrem niedrigen Transaktionskosten, komplett abgesichert durch das Ethereum-Mainnet. Kein Server-Hack kann die Herkunft Ihrer Produkte jemals verfälschen.</p><h4>3. Smart Contracts & Royalty Engine</h4><p>Ihre Investition wandelt sich in Profit: Smart Contracts triggern bei jedem Weiterverkauf der Ware automatisiert eine Zahlungsfunktion. Die Logik entscheidet on-chain, wer die Royalty-Fees erhält.</p><h4>4. Account Abstraction (Invisible Wallets)</h4><p>Wir verwenden den ERC-4337 Standard zur Account Abstraction. Alle Gas-Fees werden im Backend von unserem Paymaster subventioniert. So erleben Ihre Endkunden und Sie selbst das System exakt wie normales Online-Banking. Keine Seed-Phrases, stattdessen Auszahlungen direkt in Fiat oder USDT – komplexe Kryptographie wird endlich unternehmenstauglich.</p>"
        }
    },
    "en-us": {
        ...
        "tech": {
            ...
            "content": "<h3>Next-Generation Architecture</h3><p>We unite state-of-the-art Web3 software with robust hardware to build a flawlessly secure European export highway. As a SaaS provider, we handle the entire backend complexity, whilst you eliminate compliance risks and generate new revenue.</p><hr class='border-white/10 my-6'><h3>The Tech Stack in Detail</h3><h4>1. NXP NTAG 424 DNA (Physical Anchor)</h4><p>QR codes are worthless against professional piracy. We use cryptographically secure NFC chips (AES-128) from NXP. Every time a product is scanned, it generates a unique, dynamic crypto code. This creates a tamper-proof bridge between the physical goods and its Digital Product Passport (DPP).</p><h4>2. Base L2 Blockchain (Secure Ledger)</h4><p>The digital product identity is immortalized on <strong>Base (Ethereum Layer 2)</strong>. This guarantees unprecedented scalability at extremely low transaction costs, inheriting the security of the Ethereum mainnet. No server hack can ever fake the provenance of your goods.</p><h4>3. Smart Contracts & Royalty Engine</h4><p>Your investment turns into profit: Custom-written smart contracts automatically trigger a payment function upon every resale of the goods. The logic autonomously dictates on-chain who receives the royalty fee.</p><h4>4. Account Abstraction (Invisible Wallets)</h4><p>We utilize the ERC-4337 standard for Account Abstraction. All gas fees are subsidized by our proprietary backend paymaster. Consequently, you and your clients experience the system exactly like conventional online banking. No seed phrases, just direct payouts in fiat or USDT — making complex cryptography enterprise-ready.</p>"
        }
    },
    "vi-vn": {
        ...
        "tech": {
            ...
            "content": "<h3>Kiến trúc Thế hệ Mới</h3><p>Chúng tôi kết hợp phần mềm Web3 với phần cứng mạnh mẽ. Là một nhà cung cấp SaaS, chúng tôi xử lý sự phức tạp, còn bạn loại bỏ rủi ro tuân thủ và tạo doanh thu mới.</p><hr class='border-white/10 my-6'><h3>Phân tích Tech Stack</h3><h4>1. NXP NTAG 424 DNA (Mỏ neo Vật lý)</h4><p>Mã QR vô giá trị trước hàng giả tinh vi. Chúng tôi dùng chip NFC mã hóa (AES-128). Mỗi lần quét tạo ra một mã crypto động duy nhất. Đây là cầu nối chống giả mạo sang Hộ chiếu Sản phẩm Số (DPP).</p><h4>2. Base L2 Blockchain (Sổ cái Bảo mật)</h4><p>Định danh sản phẩm được lưu mãi mãi trên <strong>Base (Ethereum Layer 2)</strong>. Đảm bảo mở rộng cao với chi phí giao dịch siêu thấp. Không hacker nào có thể làm giả nguồn gốc của bạn.</p><h4>3. Smart Contracts & Động cơ Bản quyền</h4><p>Đầu tư tạo lợi nhuận: Hợp đồng thông minh tự động kích hoạt thanh toán ở mọi lần bán lại hàng hóa, phân bổ doanh thu on-chain chuẩn xác.</p><h4>4. Account Abstraction (Ví Vô hình)</h4><p>Chúng tôi sử dụng chuẩn ERC-4337. Mọi phí gas đều được backend của chúng tôi tài trợ. Bạn và khách hàng trải nghiệm như ngân hàng điện tử, nhận payout qua fiat hoặc USDT dễ dàng.</p>"
        }
    },
    "th-th": {
        ...
        "tech": {
            ...
            "content": "<h3>สถาปัตยกรรมระดับถัดไป</h3><p>เราผสานซอฟต์แวร์ Web3 เข้ากับฮาร์ดแวร์ที่แข็งแกร่ง ในฐานะผู้ให้บริการ SaaS เราจัดการความซับซ้อนให้คุณทั้งหมด ป้องกันความเสี่ยงตามกฎระเบียบและสร้างรายได้</p><hr class='border-white/10 my-6'><h3>Tech Stack อย่างละเอียด</h3><h4>1. NTAG 424 DNA (การเชื่อมโยงกายภาพ)</h4><p>คิวอาร์โค้ดไร้ค่าต่อการปลอมแปลงระดับมืออาชีพ เราใช้ชิป NFC (AES-128) ที่ปลอดภัยทางรหัสผ่านจาก NXP เมื่อมีการสแกนจะสร้างโค้ดแบบไดนามิก ป้องกันการปลอมแปลงร้อยเปอร์เซ็นต์</p><h4>2. Base L2 Blockchain</h4><p>การระบุตัวตนบนสินค้าถูกเข้ารหัสบน <strong>Base L2</strong> ตลอดกาล มีความสามารถในการขยายระบบสูงมากด้วยต้นทุนการทำธุรกรรมที่ต่ำ</p><h4>3. Smart Contracts & Royalty Engine</h4><p>ก้าวข้ามต้นทุนสู่ผลกำไร: สัญญาอัจฉริยะทำให้การชำระเงินเกิดขึ้นโดยอัตโนมัติเมื่อมีการขายต่อสินค้า แจกแจงผลตอบแทนผ่าน On-Chain ทันที</p><h4>4. Account Abstraction (Invisible Wallets)</h4><p>มาตรฐาน ERC-4337 ใช้ Account Abstraction เพื่อขจัดภาระของผู้ใช้งาน ค่าธรรมเนียมทั้งหมดถูกชำระโดยระบบหลังบ้าน ไม่ต้องมีรหัสผ่าน ให้ประสบการณ์ดั่งระบบธนาคารออนไลน์</p>"
        }
    },
    "zh-cn": {
        ...
        "tech": {
            ...
            "content": "<h3>下一代科技架构</h3><p>我们将绝妙的 Web3 协议与坚固的硬件融合，建立无懈可击的安全出口网络。作为 SaaS 提供商，我们为您接管后端所有的架构复杂性。</p><hr class='border-white/10 my-6'><h3>架构剖析 (The Tech Stack)</h3><h4>1. 恩智浦 NTAG 424 DNA (物理防伪重锚)</h4><p>纯二维码在专业造假面前不堪一击。我们采用物理级安全加密的 NFC 芯片 (AES-128)。每次扫描均产生极为复杂的动态变化密码，构建真正通往数字护照 (DPP) 的防伪屏障。</p><h4>2. Base L2 区块链网络 (安全账本)</h4><p>让产品身份长存于 <strong>Base (以太坊二层网络)</strong>。在极低的每笔交易手续费下拥有极高的全网安全扩展性，杜绝服务器遭黑客改写源头的悲剧。</p><h4>3. 智能合约与版税引擎</h4><p>合规不再纯烧钱：智能合约自发执行在产品于欧洲的每一次转售中收取版税的过程。不可修改的代码即规则，实时链上决策分成。</p><h4>4. 账户抽象化 (隐形钱包)</h4><p>我们采用最新的 ERC-4337 标准实现账户抽象化交易。所有的 Gas (矿工交易费) 统统由我们的后端付款代理出资补偿贴现。告别助记词学习成本，以使用传统网上银行直充法币或 USDT 的体验进行企业结算全流程。</p>"
        }
    },
    "zh-tw": {
        ...
        "tech": {
            ...
            "content": "<h3>下一代科技架構</h3><p>我們將絕妙的 Web3 協議與堅固的硬體融合，建立無懈可擊的安全出口網絡。作為 SaaS 提供商，我們為您接管後端所有的架構複雜性。</p><hr class='border-white/10 my-6'><h3>架構剖析 (The Tech Stack)</h3><h4>1. 恩智浦 NTAG 424 DNA (物理防偽重錨)</h4><p>純二維碼在專業造假面前不堪一擊。我們採用物理級安全加密的 NFC 晶片 (AES-128)。每次掃描均產生極為複雜的動態變化密碼，構建真正通往數位護照 (DPP) 的防偽屏障。</p><h4>2. Base L2 區塊鏈網絡 (安全帳本)</h4><p>讓產品身份長存於 <strong>Base (以太坊二層網絡)</strong>。在極低的每筆交易手續費下擁有極高的全網安全擴展性，杜絕伺服器遭駭客改寫源頭的悲劇。</p><h4>3. 智能合約與版稅引擎</h4><p>合規不再純燒錢：智能合約自發執行在產品於歐洲的每一次轉售中收取版稅的過程。不可修改的代碼即規則，實時鏈上決策分成。</p><h4>4. 帳戶抽象化 (隱形錢包)</h4><p>我們採用最新的 ERC-4337 標準實現帳戶抽象化交易。所有的 Gas (礦工交易費) 統統由我們的後端付款代理出資補償貼現。告別助記詞學習成本，以使用傳統網上銀行直充法幣或 USDT 的體驗進行企業結算全流程。</p>"
        }
    }
};
